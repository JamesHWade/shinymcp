---
title: "Automatic Shiny App Conversion"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Automatic Shiny App Conversion}
...
This vignette covers the `convert_app()` pipeline and each of its
stages.
...
| `tools.R` | `ellmer::tool()` definitions for each reactive group |
...
`parse_shiny_app()` reads the app's R source and extracts a structured
...
and type), the body of the `server` function, each `reactive()`
expression and the inputs it depends on, each `observe()` and
`observeEvent()` call, and every `input$name` and `input[["name"]]`
reference.
...
`analyze_reactive_graph()` takes the IR and builds a dependency graph:
...
So a Shiny app where a `reactive()` feeds both `renderPlot()` and
`renderText()` produces a single tool group that holds all the related
...
It generates the UI code using shinymcp components (`mcp_select()`,
`mcp_plot()`, and so on) mapped from the original Shiny inputs and
outputs. It writes the tool definitions as `ellmer::tool()` calls with
typed arguments (`type_string`, `type_number`, `type_boolean`) and tool
annotations. It writes a server entrypoint that sources the tools and
sets up a state environment. For complex apps, it also writes conversion
notes listing what needs manual review.
...
| `radioButtons()` | `mcp_radio()` (or `mcp_select()`) |
| `actionButton()` | `mcp_action_button()` |
| `plotOutput()` | `mcp_plot()` with base64 PNG rendering |
| `textOutput()` / `verbatimTextOutput()` | `mcp_text()` |
| `tableOutput()` | `mcp_table()` |
| `htmlOutput()` / `uiOutput()` | `mcp_html()` |
...
`mcp_html()` and return HTML strings from the tool:
...
`mcp_text_input()` for file paths instead, or have the tool read from a
known directory.

## Controlling output

By default, `convert_app()` writes to `{path}_mcp/`. Specify a custom
output directory:
...
the automatic pipeline), see `vignette("converting-shiny-apps")`.
