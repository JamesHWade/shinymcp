...
`initialize`, shinymcp serves your tools with no UI metadata and they run
as plain text tools. That is graceful degradation, not a bug. Check that
...
default, and they fail with no error you can see. Either inline everything
(shinymcp's default behavior covers htmltools dependencies automatically)
or declare the domains via `mcp_app(csp = list(connect_domains = ..., resource_domains = ...))`.
