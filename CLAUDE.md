...
from `<script id="shinymcp-config">` element - Listens for input changes
→ sends `ui/input-changed` to host - Receives `ui/tool-result` → updates
DOM output elements - Reports size changes via ResizeObserver - Handles
teardown cleanup
...
2.  **One tool per reactive group** - Connected inputs/reactives/outputs
    map to a single tool.
3.  **Self-contained JS** - No npm build step. Vanilla JS in an IIFE.
4.  **mcptools extension path** - Resource handling in `mcp-resources.R`
